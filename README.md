...
[`collateral/instagram-2026-09/`](./collateral/instagram-2026-09/) holds the Instagram launch set:
five 1080×1350 cards for one carousel, with the caption and the alt text for each slide. The cards
are generated by `build-cards.mjs`, which reads their text from `content/`, `nav.json` and
`site.config.json` (only two short headlines are written into the builder). It captures the home
page from `docs/` and three live sites: the two starters and history-of-indigo. It refuses to write
a card that fails its font, contrast, type-size, crop or layout checks. It needs Playwright with
Chromium (not a dependency of this repo), and nothing goes into `docs/`.

```bash
node collateral/instagram-2026-09/build-cards.mjs      # all five cards, alt text and proof
```

Its [README](./collateral/instagram-2026-09/README.md) says where each card's words come from.
